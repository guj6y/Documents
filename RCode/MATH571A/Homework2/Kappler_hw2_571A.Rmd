...
For $\alpha = `r alpha7e`$, the critical t-value is `r tval7e`.  R gives $t^* =`r sqrt(BF7.test[1])`$ (also, we have that $p=`r BF7.test[2]`>`r alpha7e`).
.  We fail to reject the null hypothesis that the variance is constant.  There is not sufficient evidence to show that the variance in the simple linear model predicting Mean Muscle Mass from age is significantly nonconstant.  This does not support my preliminary feelings from c.
